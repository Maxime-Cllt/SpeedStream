...
pub async fn insert_speed_data(
    ...
    const QUERY_INSERT: &str = "INSERT INTO speed (sensor_name,speed,lane) VALUES (NULLIF($1, ''), $2, $3) RETURNING id, sensor_name, speed, lane, created_at";

    let query = sqlx::query_as::<_, SpeedData>(QUERY_INSERT)
        .bind(payload.sensor_name.unwrap_or_default())
        .bind(payload.speed)
        .bind(i32::from(payload.lane))
        .fetch_one(db);

    with_timeout(query).await.map_err(|e| {
...
pub async fn fetch_last_n_speed_data(
    ...
    let rows: Vec<SpeedData> = with_timeout(query).await.map_err(|e| {
...
pub async fn fetch_last_speed(db: &PgPool) -> Result<SpeedData, sqlx::Error> {
    ...
    let query = sqlx::query_as::<_, SpeedData>(QUERY).fetch_one(db);

    let row: SpeedData = with_timeout(query).await.map_err(|e| {
